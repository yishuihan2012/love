...
class IndexController extends Controller {
...

    {
       ...
       /* $login_type=session('login_type');
        ...
        $count['count_member']=M('usermanage')->count();
        $t = time();
        $count['time']=$t;
        $start_time = mktime(0,0,0,date("m",$t),date("d",$t),date("Y",$t));  //当天开始时间
        $end_time = mktime(23,59,59,date("m",$t),date("d",$t),date("Y",$t)); //当天结束时间
        $count['count_member_today']=M('usermanage')->where('u_times between "'.date('Y-m-d H:i:s',$start_time).' " and "'.date('Y-m-d H:i:s',$end_time).'"')->count();
        $count['count_order']=M('quickpayorder')->count();
        $count['count_success_order']=M('quickpayorder')->where(array('qp_qfsatus'=>'SUCCESS'))->count();
        // print_r($count);die;
