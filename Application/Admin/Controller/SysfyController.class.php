...
class SysfyController extends ConController { //迅富钱包比例配置
    public function index(){
      if(IS_POST){ //编辑操作
        ...
        $DB=M('sysxfmanage');
        $configs=$DB->where(array('proxy'=>session('id')))->find();
        if($configs){
          $DB->where(array('proxy'=>session('id')))->save($data);
        }else{
          $DB->add($data);
        }
        echo 1;
        ...
        $sysconf=$DB->where(array('proxy'=>session('id')))->find();//根据ID找到对应的 配置信息
        //
    ...
    public function modifyLoginPasswd_act()
    {
    	...
    	//对比输入的旧密码是否一致
    	$original_passwd = md5( md5($original_passwd) . 'MRPI' );
        if($identify==1){
    	...
    	//修改密码
    	$new_passwd = md5( md5($new_passwd) . 'MRPI' );
